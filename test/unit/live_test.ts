...
import {WebSocket, WebSocketCallbacks, WebSocketFactory} from '../../src/_websocket';
...
describe('live', () => {
  it('connect uses default callbacks if not provided', async () => {
    const apiClient =
        new ApiClient({auth: new FakeAuth(), apiKey: 'test-api-key'});
    const websocketFactory = new FakeWebSocketFactory();
    const live = new Live(apiClient, new FakeAuth(), websocketFactory);

    let websocketFactorySpy =
        spyOn(websocketFactory, 'create').and.callThrough();
    ...
    expect(websocketFactorySpyCall.args[0])
        .toBe(
    ...
    const onopenString =
        JSON.stringify(websocketFactorySpyCall.args[2].onopen.toString());
    expect(onopenString)
        .toContain(
            '(_a = callbacks === null || callbacks === void 0 ? void 0 : callbacks.onopen) === null || _a === void 0 ? void 0 : _a.call(callbacks);');
    expect(onopenString).toContain('onopenResolve();');
    expect(JSON.stringify(websocketFactorySpyCall.args[2].onmessage.toString()))
        .toBe('"function (e) { }"');
    expect(JSON.stringify(websocketFactorySpyCall.args[2].onerror.toString()))
        .toBe('"function (e) { }"');
    expect(JSON.stringify(websocketFactorySpyCall.args[2].onclose.toString()))
        .toBe('"function (e) { }"');
  ...
  it('connect should rely on provided callbacks', async () => {
    ...
    try {
      await live.connect('models/gemini-2.0-flash-exp', {}, {
        onopen: () => {
  ...
  it('connect should send setup message', async () => {
    ...
    expect(websocketSpyCall.args[0])
        .toBe('{"setup":{"model":"models/gemini-2.0-flash-exp"}}');
    expect(session).toBeDefined();
  });
});
